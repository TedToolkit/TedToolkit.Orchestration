...
internal static class StepExecutionEmitter
{
    ...
    internal static Method CreateRun(GraphNode node, bool parallel)
    {
        var factory = node.Factory;
        var policy = factory.RetryCount != 0 || factory.TimeoutMilliseconds != -1;
        ...
        if (parallel)
        {
            ...
            if (dependencies.Length != 0) method.AddStatement(Call("executionToken.ThrowIfCancellationRequested"));
        }
        var arguments = PrepareArguments(method, node);
        ...
        if (policy)
        {
            var lifetime = new UsingStatement(Variable("attempt", New(new DataType("StepAttempt"),
                factory.RetryCount.ToLiteral(), factory.TimeoutMilliseconds.ToLiteral(), Name("executionToken"))));
        ...
        if (parallel && !policy)
            AppendAttempt(body, node, arguments, "executionToken", false, false);
        else
        {
            var execution = new TryStatement();
            AppendAttempt(execution, node, arguments, policy ? "attempt.Token" : "executionToken", policy, directTask);
        ...
        if (parallel)
        {
            ...
            guard.AddCatch(new CatchClause(typeof(System.Exception))
                .AddStatement(Await(Call("CancelRemainingAsync", Name("__execution"))))
    ...
    private static IExpression[] PrepareArguments(Method method, GraphNode node)
    {
        var factory = node.Factory;
        ...
        var arguments = ConstructorArguments(node).ToArray();
    ...
    private static void AppendAttempt(IStatementOwner body, GraphNode node, IExpression[] arguments, string token, bool policy, bool directTask)
    {
        var factory = node.Factory;
        ...
        if (policy ? factory.TimeoutMilliseconds != -1 :
            node.Locals.Any() || node.Arguments.Any(argument => argument.Expression is not null && !argument.IsConstant) ||
        ...
        var disposable = factory.IsSynchronous && (factory.Type.GetMembers("Dispose").Length != 0 ||
        ...
        IExpression call = Call((disposable ? Name("step") : (IExpression)creation).Sub(factory.IsSynchronous ? "Execute" : "ExecuteAsync"), Name(token));
        if (directTask)
        {
            execution.AddStatement(Call("ObserveStepAsync", call, Name(token)).Return);
            return;
        }
    ...
    private static IEnumerable<IExpression> ConstructorArguments(GraphNode node)
    {
        var argumentIndex = 0;
